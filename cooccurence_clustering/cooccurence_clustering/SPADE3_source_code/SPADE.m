...
if length(all_fcs_filenames)~=0 && exist(fullfile(directoryname,'cytobank_data_source.m'))
    fprintf('FCS files already exist in selected directory, no need to download from cytobank_data_source.m!\n\n');
end
if length(all_fcs_filenames)==0 % if this is a valid directory, but has no fcs file in it
    if ~exist(fullfile(directoryname,'cytobank_data_source.m')) || download_convert_data_from_cytobank(directoryname)==0
        % ??? clear all internal variables
        if ~exist(fullfile(directoryname,'cytobank_data_source.m'))
            fprintf('There are %d fcs files in this directory!\n\n',length(all_fcs_filenames));
        else
            fprintf('Download from cytobank failed!!\nInformation in cytobank_data_source.m does not work!!\n\n');
            fprintf('There are %d fcs files in this directory!\n\n',length(all_fcs_filenames));
        end
        handles.all_fcs_filenames = cell(0);
        handles.file_annot = cell(0); 
        ...
        guidata(hObject, handles);
        return
    else % exist the cytobank_data_source.m file and download convert is successful
        all_fcs_filenames = getfilenames(handles.directoryname);
        all_fcs_filenames = all_fcs_filenames(isInListEnd(all_fcs_filenames,'.fcs'));
    end
end
