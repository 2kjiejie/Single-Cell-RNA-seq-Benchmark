...
if ~isempty(header)
    if size(header,1)==1
        ...
        for i=1:size(header,1)
            header_row_tmp = cell2mat(strcat(header(i,:),{sep})); header_row_tmp(end)=char(10);
            header_row = [header_row,header_row_tmp];
        end
    end
...
for i=1:row_block_size:total_rows
    ...
    if isempty(data)
        ...
        data_tmp = reshape(regexp(data_tmp_str,sep,'split'),num_rows,num_columns);
        clear data_tmp_str
    end
    ...
    if isempty(left_columns)
        ...
        left_columns_tmp = left_columns(i:min(total_rows,i+row_block_size-1),:);
    end
...
%     data_str = num2str(reshape(data,1,num_rows*num_columns));
%     data_str(regexp(num2str(data_str),' +','start'))=sep;
%     data_str(data_str==' ')=[];
%     data = reshape(regexp(data_str,char(9),'split'),num_rows,num_columns);
%     clear data_str
% end
% 
% % merge left_column and data_column and write
% left_columns = [left_columns, data];
% num_columns = size(left_columns,2);
% num_rows = size(left_columns,1);
% left_columns_str = cell2mat(reshape(strcat(left_columns,{sep})',1,num_columns*num_rows));
% ind = find(left_columns_str==sep);
% left_columns_str(ind(num_columns:num_columns:end)) = char(10);
% fwrite(fid,left_columns_str,'char');
% fclose(fid);
% return