function table_content = read_xls_v3(filename, sep, skip_rows, column_ind, num_rows)
% table_content = read_xls_v3(filename, sep, skip_rows, column_ind, num_rows)
...
fid = fopen(filename);
ftext = textscan(fid, '%s', 1, 'delimiter', '\n', 'headerlines', skip_rows+1,'BufSize',BufferSize);
if iscell(ftext{1})
    tmp = ftext{1}{1}; 
else
    tmp = ftext{1};
...
if total_lines-skip_rows <=chunkSize
    fid = fopen(filename);
    if skip_rows~=0
        ftext = textscan(fid, '%s', 1, 'delimiter', '\n', 'headerlines', skip_rows-1, 'BufSize',BufferSize); % skip the "skip_rows"
    end
    ftext = textscan(fid, '%s', (total_lines-skip_rows)*num_columns, 'delimiter', sep, 'BufSize',BufferSize);
    ...
    while 1
        ...
        ftext = textscan(fid, '%s', this_chunkSize*num_columns, 'delimiter', sep, 'BufSize',BufferSize);
